...
const MovieList = (props) => {
        ...
        <div className='row'>
            ...
            <div className='col-lg-4' key={i}>
                ...
                <img src={movie.imageURL} className='card-img-top' alt={movie.name}></img>
                <div className='card-body'>
                    <h5 className='card-title'>{movie.name}</h5>
                    <p className='card-text'>{truncateOverview(movie.overview,200)} </p>
                    <div className='d-flex justify-content-between align-items-center'>
                        <button type='button' className='btn btn-md btn btn-outline-danger' onClick={(event)=> props.deleteMovie(movie)}>Delete</button>
